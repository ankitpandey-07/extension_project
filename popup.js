document.getElementById("summarizer").addEventListener("click", async () => {
  ...
  chrome.storage.sync.get(["geminiApiKey"], async (result) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
      chrome.tabs.sendMessage(tab.id, { type: "GET_ARTICLE_TEXT" }, async (res) => {
        ...
        try {
          const translated = await translateText(res.text, result.geminiApiKey, targetLang);
          resultDiv.innerText = translated;
        } catch (err) {
          resultDiv.innerText = `Translation failed: ${err.message}`;
        }
      });
    });
  });
...
async function translateText(text, apiKey, targetLang) {
  ...
  try {
    ...
    const data = await res.json();
    return data?.candidates?.[0]?.content?.parts?.[0]?.text || "No translation available.";
  } catch (error) {
